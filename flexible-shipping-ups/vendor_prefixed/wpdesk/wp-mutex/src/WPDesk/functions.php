...
{
    return new \UpsFreeVendor\WPDesk\Mutex\WordpressMySQLLockMutex($lockName, $waitForLockTimeout);
}
/**
...
{
    return \UpsFreeVendor\WPDesk\Mutex\WordpressMySQLLockMutex::fromOrder($order, $lockName, $waitForLockTimeout);
}
/**
 * Acquire lock.
 *
...
{
    if ('mysql' === $lockType) {
        $mutex = \UpsFreeVendor\wpdesk_create_mysql_lock($lockName, $waitForLockTimeout);
        $storage = new \UpsFreeVendor\WPDesk\Mutex\StaticMutexStorage();
