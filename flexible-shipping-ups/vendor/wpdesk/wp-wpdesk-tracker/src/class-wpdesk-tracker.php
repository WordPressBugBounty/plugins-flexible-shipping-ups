...
if ( ! class_exists( 'WPDesk_Tracker' ) ) {
	...
	class WPDesk_Tracker implements WPDesk_Tracker_Interface {
		...
		public function admin_bar_menu( $wp_admin_bar ) {
			if ( apply_filters( 'wpdesk_tracker_use_testdata', false ) && apply_filters(
				'wpdesk_tracker_show_testdata',
				false
			) ) {
				$args = [
					'id'    => 'my_page',
					'title' => 'WP Desk Test!',
					'meta'  => [ 'class' => 'wpdesk-tracker-test' ],
				];
		...
		public function admin_enqueue_plugins_scripts(): void {
			...
			if ( 'plugins.php' === $pagenow && ! is_network_admin() && $this->should_enable_wpdesk_tracker() && ! apply_filters(
				'wpdesk_tracker_do_not_ask',
				false
			) ) {
				wp_register_script(
					self::ADMIN_PLUGINS_HANDLER,
					plugin_dir_url( __FILE__ ) . 'assets/js/admin_plugins.js',
					[],
					$this->script_version
				);
		...
		public function admin_enqueue_scripts(): void {
			...
			if ( $screen->id === 'admin_page_wpdesk_tracker' || $screen->id === 'admin_page_wpdesk_tracker_deactivate' ) {
				...
				wp_register_style(
					self::ADMIN_HANDLER,
					plugin_dir_url( __FILE__ ) . 'assets/css/tracker.css',
					[],
					$this->script_version,
					'all'
				);
				...
				wp_localize_script(
					...
					[
						...
						'tracker_page'         => \esc_url_raw( str_replace( "&amp;", "&", admin_url( wp_nonce_url( "plugins.php?action=deactivate&plugin=" . $plugin . "&plugin_status=all&", "deactivate-plugin_" . $plugin ) ) ) ),
						'plugin'               => esc_attr( $plugin ),
						'plugin_name'          => esc_attr( $plugin_name ),
						'submit_txt'           => esc_html__( "Submit &amp; Deactivate", "wpdesk-tracker" ),
		...
		public function admin_enqueue_notice_scripts(): void {
			...
			if ( $this->is_notices_enabled() ) {
				...
				wp_register_script(
					self::ADMIN_NOTICE_HANDLER,
					plugin_dir_url( __FILE__ ) . 'assets/js/admin_notices.js',
					[],
					$this->script_version
				);
				...
				wp_localize_script(
					self::ADMIN_NOTICE_HANDLER,
					self::ADMIN_NOTICE_LOCALIZE,
					[
						'action'               => \esc_attr( self::AJAX_NOTICE_ACTION ),
						'nonce'                => wp_create_nonce( self::WPDESK_TRACKER_NOTICE ),
						'ajax_url'             => \esc_url_raw( admin_url( 'admin-ajax.php' ) ),
						'plugin'               => esc_attr( $this->plugin_basename ),
					]
				);
			}
		}
		...
		public function admin_menu() {
			...
			add_submenu_page(
				...
				[ $this, 'wpdesk_tracker_deactivate' ]
			);
		}
		...
		public function update_option_wpdesk_helper_options( $old_value, $value, $option ) {
			...
			if ( $old_value['wpdesk_tracker_agree'] != '1' ) {
				if ( $value['wpdesk_tracker_agree'] == '1' ) {
					$this->send_tracking_data( true, 'agree', $this->read_context_from_request() );
				}
			}
			if ( $old_value['wpdesk_tracker_agree'] != '0' ) {
				if ( $value['wpdesk_tracker_agree'] == '0' ) {
					$this->send_tracking_data( true, 'no', $this->read_context_from_request() );
					update_option( 'wpdesk_tracker_notice', 'dismiss_all' );
				}
			}
		}
		...
		public function send_tracking_data( $override = false, $click_action = null, array $additional_data = [] ) {
			...
			if ( ! apply_filters( 'wpdesk_tracker_send_override', $override ) ) {
				...
				if ( $last_send && $last_send > apply_filters(
					'wpdesk_tracker_last_send_interval',
					strtotime( '-1 week' )
				) ) {
					return;
				}
		...
		private function can_display_notice(): bool {
			if ( has_filter( 'wpdesk_tracker_notice_screens' ) === true ) {
				$screen = get_current_screen();
				return in_array( $screen->id, apply_filters( 'wpdesk_tracker_notice_screens', [] ), true );
			}
